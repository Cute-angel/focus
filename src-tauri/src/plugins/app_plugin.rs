...
impl AppPlugin {
    ...
    fn get_commands(&self) -> CommandNode {
        ...
        let show_restart_app =
            move |_ctx: CommandContext, _app: AppHandle| restart_res.clone().into();
        let show_stop_app = move |ctx: CommandContext, app: AppHandle| stop_res.clone().into();
        ...
        CommandNode::new("manager")
            .then(CommandNode::new("restart").execute(show_restart_app))
            .then(CommandNode::new("stop").execute(show_stop_app))
            .execute(show_app)
    }
}
...
impl Extension for AppPlugin {
    ...
    fn on_plugin_load(&self, core: &mut Core) {
        core.get_command_dispatcher().register(self.get_commands());
        core.get_action_runner()
            .add("app_manager", self.get_plugin_action());
    }
}
